...
void partie_en_cours()
{
    ...
    remplissage_chance_commu(tabcarteschance, tailletabcartes);
    remplissage_chance_commu(tabcartescommu, tailletabcartes);
    ...
    while (nombre_de_joueurs != 1)
    {
        ...
        {
            fin_de_partie = tour_joueur( x, repere, joueur, nombre_de_joueurs, i, tab, identifiant_carte, tab_c, loto, card_chance, card_commu, sauvegarde_position, banque_de_carte, tour_prison);
            if (fin_de_partie <= 0)
            {
