        ...
        for (int j = 0; j < nb_cartes; j++)
        {
            ...
            if (terrain[i][j].id_carte[id_joueur] == touche && terrain[i][j].possession_carte == id_joueur)
            {
                ...
                while (fleche2 != TOUCHE_ENTER) // Tant que ENTER n'est pas pressé, on boucle à l'infini
                {
                    ...
                    if ((fleche2 == 'z' || fleche2 == 'Z') && terrain[i][j].nb_maison > 0 && nb > 0) // z sert à augmenter de 1 le nombre de maison a vendre
                    {
                    ...
                    else if ((fleche2 == 's' || fleche2 == 's') && terrain[i][j].nb_maison < 4 && nb < 4) // s sert a diminuer de 1 le nombre de maisons à vendre
                    {
                        terrain[i][j].nb_maison++;
                        terrain[i][j].prix_loyer = terrain[i][j].prix_loyer * 2 + 50;
                    ...
                    else
                    {
                        carre_noir();
                        Color(id_joueur + 1, 0);
                        char echec_vente2[100] = ", vous ne possedez aucune maison sur cette propriete !";
